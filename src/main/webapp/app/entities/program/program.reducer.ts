...
export const ACTION_TYPES = {
  SEARCH_PROGRAMS: 'program/SEARCH_PROGRAMS',
...
export default (state: ProgramState = initialState, action): ProgramState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_PROGRAMS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_PROGRAMS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_PROGRAMS):
...
const apiSearchUrl = 'api/_search/programs';

// Actions

export const getSearchEntities: ICrudSearchAction<IProgram> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_PROGRAMS,
  payload: axios.get<IProgram>(`${apiSearchUrl}?query=${query}`)
});
